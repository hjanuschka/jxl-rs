...
fn encode_single_rgba_frame(
    ...
    let x_dm_multiplier = (1.0f32 / 1.25).powf(3.0 - 2.0) * 1.0625; // = 0.85, even coarser X
...
fn build_adaptive_raw_quant_map_full(
    ...
    let num_blocks = bw * bh;
    let quant_lf_boost: u32 = if num_blocks <= 7000 { 4 } else { 0 };
...
fn write_vardct_frame_header_full(writer: &mut BitWriter, cfg: &FrameHeaderConfig) -> Result<()> {
    ...
    writer.write(16, f32_to_f16_bits(6.0))?;  // epf_channel_scale[0] = 6.0 (Y, test with ep=2/qlf+4)
