...
impl<S: JxlState> JxlDecoder<S> {
    ...
    /// Returns parsed JPEG reconstruction data (`jbrd` box), if present.
    pub fn jpeg_reconstruction_data(&self) -> Option<&JpegReconstructionData> {
        self.inner.jpeg_reconstruction_data()
    }

    /// Returns true if the container had a JPEG reconstruction (`jbrd`) box.
...
pub(crate) mod tests {
    ...
    #[test]
    fn test_jpeg_reconstruction_box_parsed_from_container() {
        fn make_box(ty: &[u8; 4], content: &[u8]) -> Vec<u8> {
            let len = (8 + content.len()) as u32;
            let mut buf = Vec::new();
            buf.extend(len.to_be_bytes());
        ...
        let jbrd_payload = vec![1u8, 2, 3, 4, 5, 6, 7, 8];

        let sig = [
            0x00, 0x00, 0x00, 0x0c, 0x4a, 0x58, 0x4c, 0x20, 0x0d, 0x0a, 0x87, 0x0a,
        ];
        let ftyp = make_box(b"ftyp", b"jxl \x00\x00\x00\x00jxl ");
        let jbrd = make_box(b"jbrd", &jbrd_payload);
        let jxlc = make_box(b"jxlc", &codestream);

        let mut container = Vec::new();
        container.extend(&sig);
        container.extend(&ftyp);
        container.extend(&jbrd);
        container.extend(&jxlc);
