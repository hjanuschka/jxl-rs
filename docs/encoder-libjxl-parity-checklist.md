...
- `[~]` Early real image-to-bitstream path: RGB8 raw input into modular stream (group-aware bootstrap path, interleaved + strided API inputs).
...
- [~] A01 Add `JxlEncoder::encode_image(...)` for raw in-memory buffers (RGB/Gray/RGBA, u8/u16/f32).
  - Current: `encode_modular_u8_rgb_codestream/container` + `encode_image` for RGB8 interleaved/strided inputs, group-aware bootstrap path.
